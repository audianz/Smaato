...
function smaato_request($request)
{
	...
	else
	{
		...
		$bid_floor=empty($request_array['imp'][0]['bidfloor'])?0:$request_array['imp'][0];
		...
		if(!empty($request_array['imp']))
		{
			...
			$get_bannerInfo= mysql_query("SELECT * FROM ox_banners as b, ox_campaigns as c, ox_clients as a, oxm_accbalance as bal, oxm_budget as d
					WHERE
					b.width=".$width." AND
					b.height=".$height." AND
					...
					GROUP BY b.master_banner")or die(mysql_error());

	 	/**
	 	 *  $get_bannerInfo query checked the width and height exact and less than exact size
	 	 *
	 	 * group by master banenr
	 	 *
	 	* */

			if(mysql_num_rows($get_bannerInfo)>0)
			{
					
				$date=date("Y-m-d");

				while($get_bannerRow=mysql_fetch_array($get_bannerInfo))
				{
					$banner_rate=$get_bannerRow['revenue']/1000;
					$bannerid=$get_bannerRow['bannerid'];

					$campaign_query=mysql_query("SELECT SUM(amount) as daily_amount FROM oxm_report WHERE campaignid='".$get_bannerRow['campaignid']."' AND DATE(date)='".$date."'")or die(mysql_error());
			...
			//	$log->logDebug("bannerInfo before targeting time");
				...
				foreach($targe_ban as $binfo)
				{
					...
					if(!empty($make))
					{
						$camtagreting=mysql_query("SELECT manufacturer FROM djx_targeting_limitations WHERE campaignid=".$binfo['campaignid'])or die(mysql_error());							
						$cam_targetvalue=mysql_fetch_array($camtagreting);
					...
					if($lat!=0 && $lon!=0)
					{
						//putlog("\n location targeting lat is ".$lat."  lon is ".$lon);
							
						$camtagreting=mysql_query("SELECT enable_loc,landing_page,intermediate FROM djx_targeting_limitations WHERE campaignid=".$binfo['campaignid'])or die(mysql_error());
						$cam_targetvalue=mysql_fetch_array($camtagreting);
						$banner_enable_loc = strtolower($cam_targetvalue['enable_loc']);
							
						//putlog("\n banner location is ".$banner_enable_loc);
						if(!empty($banner_enable_loc) && $banner_enable_loc!='NULL' && $banner_enable_loc!='null' && $banner_enable_loc!='')
						{
							if($banner_enable_loc=='spec')
							{
								$res=_adLocationMatches($banid,$advid,$lat,$lon);
								...
								{
									//putlog("\n res is false unset banner id ".$banid);
									unset($banner_Infoarray[$banid]);
								}	
							}
							else
							{
								//putlog("\n banner location is all");
							}
						}
					...
					{
						//putlog("\n location lat is ".$lat."  lon is ".$lon);
						$camtagreting=mysql_query("SELECT enable_loc FROM djx_targeting_limitations WHERE campaignid=".$binfo['campaignid'])or die(mysql_error());
				 ...
				 /* Device OS  Targeting 	*/
				 if(!empty($os)){
				 	$camtagreting=mysql_query("SELECT devices FROM djx_targeting_limitations WHERE campaignid=".$binfo['campaignid'])or die(mysql_error());
				 ...
				 if(!empty($model)){
				 	$camtagreting=mysql_query("SELECT model FROM djx_targeting_limitations WHERE campaignid=".$binfo['campaignid'])or die(mysql_error());
				 ...
				 if(!empty($ip)){
				 	$camtagreting=mysql_query("SELECT carriers FROM djx_targeting_limitations WHERE campaignid=".$binfo['campaignid'])or die(mysql_error());
				 	...
				 	if(!empty($cam_targetvalue['carriers']) && $cam_targetvalue['carriers']!='NULL' && $cam_targetvalue['carriers']!='null' && $cam_targetvalue['carriers']!=''){
				 		...
				 		$ip_carrier_Info=mysql_query("SELECT * FROM ox_carrier_detail WHERE  id IN (".$cam_targetvalue['carriers'].") AND INET_ATON(start_ip) <= INET_ATON('".$ip."') AND INET_ATON(end_ip) >= INET_ATON('".$ip."')")or die(mysql_error());
				 		if(mysql_num_rows($ip_carrier_Info)>0)
				 		{
				 			$ip_carrier=mysql_fetch_array($ip_carrier_Info);
				 		}
				 ...
				 if(!empty($lat) && !empty($lon)){
				 	...
				 	$camtagreting=mysql_query("SELECT locations FROM djx_targeting_limitations WHERE campaignid=".$binfo['campaignid'])or die(mysql_error());
				 	...
				 	if(!empty($cam_targetvalue['locations']) && $cam_targetvalue['locations']!='NULL' && $cam_targetvalue['locations']!='null' && $cam_targetvalue['locations']!=''){
				 		...
				 		foreach($banner_geo as $country){
				 			...
				 			$ip_carrier=mysql_query("SELECT * FROM da_country WHERE country_code = '".$country."' AND latitude = '".$lat."' AND longitude = '".$lon."'")or die(mysql_error());
				 			 
				 			if(mysql_num_rows($ip_carrier)>0){
			...
			//	$log->logDebug("bannerinfo arr after targeting time is ");
				//putlog("\n poiIndex arr is ".print_r($poiIndex,true));
				$new_ads = array();
				...
				if(!empty($banner_Infoarray)){
					...
					if(count($same_bidad)>1){
						...
						foreach($same_bidad as $ad){
							...
							$revenue_query=mysql_query("SELECT sum(o.amount) as amount, o.campaignid as cid, c.* FROM oxm_report as o, oxm_budget as c WHERE c.campaignid='".$ad['campaignid']."' AND c.campaignid=o.campaignid")or die(mysql_error());
							$revenue_row=mysql_fetch_array($revenue_query);
			...
			//$admin_share = $second_highestbid - ($second_highestbid*($smaato_share/100));
			//$adv_bid_price = $second_highestbid;
			//$smaato_bid_price = $second_highestbid*($smaato_share/100);
        //                $log->logDebug("before query time is ");
		//	$request_query = mysql_query("SELECT id FROM djax_smaato_bid_request WHERE bid_request_id='".$request_array['id']."'");
		//	$request_row = mysql_fetch_assoc($request_query);
			$requset_id =0;//$request_row['id'];
	//	        $log->logDebug("after query time is");	
			$admin_share =empty($admin_share)?0:$admin_share;
			$response_bid=empty($response_bid)?0:$response_bid;	
		
			mysql_query("INSERT INTO  `aff_smaato_response` (
					`datetime`,
					`requset_id`,
					`id` ,
					`imp_id` ,
					...
					'".$request_array['device']['devicetype']."')") or die(mysql_error());
...
//sort the array based on banner bid rate in ascending order
...
function  _adLocationMatches($adId,$clientId,$lat,$lon)
{
	//putlog("\n inside _adLocationMatch() adid is ".$adId."  client id is ".$clientId);
	...
	//putlog("\n listofMatchedPoi is ".print_r($listofMatchedPoi,true));
	return $result;
}
...
function getShortestDistancePoi($list)
{
	...
	//putlog("\n getShortestDistancePoi poi is ".$tempPoi);
	return $tempPoi;
}
